...
| T-022 | **`.db` v2 multi-table** — schema overview, drill-in, back (D28) | Codex | `388f2f5`, merged `d78319a`. **v1 round-trip verified byte-identical** on the real `examples/demo-repo/Projects.db`, and rows survive an `addDbTable` upgrade to v2 — the check the job could not run itself, and the whole point of the read-only migration rule. Suite proven to bite: removing v1 preservation exits 1. `npm test` now **8 suites**; 19/19 E2E green |
