...
| T-016 | **Recovery UI** — expose journal list/restore over IPC and give it a surface. Without it the journal captures pre-images that nothing can reach; the mechanism is real but the feature is not usable yet | Claude | `NEEDS_USER` | `feature/T-016-recovery-ui` | `src/main/main.ts`, `preload.ts`, `electron.d.ts`, a renderer view | A user can list and restore a prior version from inside the app; **still no view-reachable route** (D20) |
