...
| --- | --- | --- | --- | --- | --- | --- |
| T-015 | **Data integrity** — pre-image write journal + restore covering every workspace write path, closing risk R4 (D19, D20) | Codex | `IN_PROGRESS` | `feature/T-015-write-journal` | new `src/main/journal.ts` + `tools/journal.test.mjs`; hooks in `src/main/main.ts` and `src/main/htmx/server.ts`; `package.json` (**`scripts` only**) | Overwrite and delete of an existing file are recoverable through the journal; restore returns exact prior bytes; retention caps enforced; journal failure never blocks a write; no journal content reachable from any view route; suite proven to fail without the journal |
...
| T-014 | **CI** — decide whether `npm run test:e2e` runs in CI. Needs a desktop session on `windows-latest` and adds minutes per run; the gate is worth little if nobody runs it locally either | Claude | `NEEDS_USER` | `.github/workflows/ci.yml` | A recorded decision either way, and CI matching it |
